...
const getPayload = (event) => {
  ...
  return body.payload || body.form_submission || body;
};

const getFormName = (payload) => (
  cleanValue(payload.form_name || payload.formName || payload.name || payload.form?.name)
);

const parseSubmissionFields = (payload) => {
  const data = payload.data || payload.human_fields || payload.fields || payload;
...
const sendLeadEmail = async (payload) => {
  const formName = getFormName(payload);
  if (formName && formName !== "lead-form") {
    return { skipped: true, reason: `Ignoring form ${formName}` };
  }
  ...
  const fields = parseSubmissionFields(payload);
...
exports.handler = async (event) => {
  try {
    const result = await sendLeadEmail(getPayload(event));
    return {
      statusCode: 200,
